...
struct StatsView: View {
        ...
        GeometryReader { proxy in
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: compact ? 14 : 20) {
                    BrandHeader(compact: compact, title: "STAT DATABASE")
                    Text("NBA PLAYER DATA · 1979–80 TO 2025–26")
                        .scoreLabel()
                        .foregroundStyle(Color.accent)
                    Text("Find a player, trace a franchise, or jump to any season.")
    ...
    private func teamSeasonCard(_ row: TeamSeason) -> some View { HStack(spacing: 12) { TeamLogo(team: row.team, season: row.season, size: 42); VStack(alignment: .leading, spacing: 3) { Text(TeamBrand.name(for: row.team)).font(.subheadline.weight(.black)); Text(row.team).scoreLabel() }; Spacer(); VStack(alignment: .trailing, spacing: 3) { Text(row.season).font(.subheadline.weight(.black)); Text("\(row.players.count) PLAYERS").scoreLabel() }; Image(systemName: "chevron.right").foregroundStyle(.white.opacity(0.38)) }.cardStyle() }
    private func leaderRow(_ entry: NBALeaderEntry, rank: Int, showsSeason: Bool) -> some View { HStack(spacing: 12) { Text("\(rank)").font(.title3.weight(.black)).foregroundStyle(rank <= 3 ? Color.accent : .white.opacity(0.65)).frame(width: 24); PlayerPortrait(player: entry.record, size: 38); VStack(alignment: .leading, spacing: 3) { Text(entry.record.playerName).font(.subheadline.weight(.black)).lineLimit(1); Text("\(entry.teamLabel)  •  \(entry.record.position)\(showsSeason ? "  •  \(entry.record.season)" : "")").font(.caption.weight(.medium)).foregroundStyle(.white.opacity(0.58)).lineLimit(1) }; Spacer(minLength: 4); TeamLogo(team: entry.record.team == "Multiple Teams" ? "NBA" : entry.record.team, season: entry.record.season, size: 24); Text(leaderValue(entry)).font(.title3.weight(.black)).foregroundStyle(Color.accent) }.cardStyle() }
...
private struct PlayerStatRow: View { let player: SeasonRecord; var body: some View { HStack(spacing: 12) { PlayerPortrait(player: player, size: 42); VStack(alignment: .leading, spacing: 3) { Text(player.playerName).font(.subheadline.weight(.black)).lineLimit(1); Text("\(player.team)  •  \(player.season)  •  \(player.position)").font(.caption.weight(.medium)).foregroundStyle(.white.opacity(0.58)) }; Spacer(minLength: 4); TeamLogo(team: player.team, season: player.season, size: 24); HStack(spacing: 9) { StatMini(value: player.points, label: "PTS"); StatMini(value: player.rebounds, label: "REB"); StatMini(value: player.assists, label: "AST") } }.padding(10).background(.white.opacity(0.065)).overlay(RoundedRectangle(cornerRadius: 14).stroke(.white.opacity(0.08))).clipShape(RoundedRectangle(cornerRadius: 14)) } }
